...
def upload(
    ...
    if len(build_files) == 1:
        ...
        b := next((f for f in build_files if f.name == build_file), None)
    ) is not None:
        build_file_path = b
    elif len(build_files) == 0:
        raise Exception("The build directory is empty")
    else:
        raise Exception("Multiple build files and no build_file parameter provided")
    ...
    web.type(
        f'https://github.com/{repo_owner}/{addon["repo_name"]}/issues',
        into="support url",
    )
